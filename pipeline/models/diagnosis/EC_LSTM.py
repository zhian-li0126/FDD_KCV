...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(x.shape[0], 64, 1)  # Reshape for LSTM input
        x, _ = self.lstm(x)  # LSTM forward pass
        x = x[:, -1, :]  # Take the last time step's output
        x = self.fc4(x)  # Final fully connected output
        return x

    def train_model(self, x, y, num_epoch=1000, k_folds=4, repeats=5, lr=1e-4, patience=50, model_save_path="data", batch_size=32):
        """
        ...
        """
        print("Training EC_LSTM for diagnosis...")
